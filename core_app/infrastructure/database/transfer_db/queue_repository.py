...
from .transfer_database import TransferDatabaseConnection

logger = logging.getLogger(__name__)

class QueueRepository:
    def __init__(self, db: TransferDatabaseConnection):
        self.db = db
    ...
    def create_main_task(self, task_data: Dict[str, Any]):
        ...
        try:
            with conn:
                conn.execute('\n                INSERT INTO main_tasks (\n                    task_id, type, is_folder, status, total_size, \n                    created_at, updated_at, local_path, remote_id, error_message\n                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n                ', (task_data['task_id'], task_data['type'], 1 if task_data.get('is_folder') else 0, task_data.get('status', 'queued'), task_data.get('total_size', 0), task_data.get('created_at'), task_data.get('updated_at'), task_data.get('local_path'), task_data.get('remote_id'), task_data.get('error_message', '')))
        except Exception as e:
    ...
    def create_sub_tasks_bulk(self, sub_tasks: List[Dict[str, Any]]):
        ...
        try:
            with conn:
                conn.executemany('\n                INSERT INTO sub_tasks (\n                    sub_task_id, main_task_id, status, local_path, \n                    remote_id, total_size, file_hash, file_details_json\n                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)\n                ', [(st['sub_task_id'], st['main_task_id'], st.get('status', 'queued'), st['local_path'], st.get('remote_id'), st.get('total_size', 0), st.get('file_hash'), json.dumps(st.get('file_details')) if st.get('file_details') else None) for st in sub_tasks])
        except Exception as e:
    ...
    def get_all_tasks(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            for row in main_rows:
                ...
                task_data = {'task_id': task['task_id'], 'type': task['type'], 'is_folder': task['is_folder'], 'status': task['status'], 'total_size': task['total_size'], 'created_at': task['created_at'], 'updated_at': task['updated_at'], 'error_message': task['error_message'], 'file_path': task['local_path'] if task['type'] == 'upload' else None, 'parent_id': task['remote_id'] if task['type'] == 'upload' else None, 'save_path': task['local_path'] if task['type'] == 'download' else None, 'db_id': task['remote_id'] if task['type'] == 'download' else None, 'child_tasks': {}}
            ...
            for row in sub_rows:
                ...
                if main_id not in tasks_map:
                    continue
                main_task = tasks_map[main_id]
                sub_data = {'status': st['status'], 'file_path': st['local_path'], 'save_path': st['local_path'], 'parent_id': st['remote_id'], 'db_id': st['remote_id'], 'total_size': st['total_size'], 'preview_msg_id': st.get('preview_msg_id'), 'transferred_parts': []}
            ...
            for row in cursor.fetchall():
                ...
                if sub_id not in sub_tasks_map:
                    continue
    ...
    def get_task(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            task_data = {'task_id': main_task['task_id'], 'type': task_type, 'is_folder': is_folder, 'status': main_task['status'], 'total_size': main_task['total_size'], 'created_at': main_task['created_at'], 'updated_at': main_task['updated_at'], 'error_message': main_task['error_message'], 'file_path': main_task['local_path'] if task_type == 'upload' else None, 'parent_id': main_task['remote_id'] if task_type == 'upload' else None, 'save_path': main_task['local_path'] if task_type == 'download' else None, 'db_id': main_task['remote_id'] if task_type == 'download' else None, 'child_tasks': {}}
            if not is_folder:
                task_data.update({'file_hash': None, 'transferred_parts': [], 'split_files_info': [], 'file_details': None})
