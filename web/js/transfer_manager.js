 ...
 * summary panel and the detailed transfer modal, and handles user interactions
...
const TransferManager = {
    ...
    addDownload(item) {
        ...
        const task = { 
            id: item.task_id, // Use UUID as the primary ID
            db_id: item.db_id, // Keep the database ID
            name: item.name, 
            size: item.size || 0, 
            progress: 0, 
            speed: 0, 
        ...
        this.downloads.set(item.task_id, task); // Use task_id as the map's key
        console.log(`[DEBUG] Added new download task. Current downloads map size: ${this.downloads.size}`);
        this.startUpdater();
    },

    // Use a unique ID for uploads instead of the filename
    addUpload(fileData) {
        // Use the task_id generated by main.js, do not generate a new one here.
    ...
    updateSummaryPanel() {
        ...
        // --- Downloads Logic (Symmetric to Uploads) ---
        ...
        // --- Unified Progress Circle for Minimized Icon ---
    ...
    cancelItem(id) {
        ...
        if (result) {
            ...
            if (map.has(key)) {
                ...
                console.warn(`Cancel failed: key "${key}" not found in map for task deletion.`);
            }
            this.tick();
        }
    },
    ...
    cancelAll() {
        const _cancelRecursively = (map) => {
            map.forEach(task => {
                // Cancel children first to ensure all sub-tasks are handled.
                if (task.isFolder && task.children) {
                    _cancelRecursively(task.children);
                }
                // Cancel the item itself. The backend primarily handles file tasks,
                // but this ensures comprehensive UI cleanup.
