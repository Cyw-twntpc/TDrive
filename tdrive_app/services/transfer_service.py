...
class TransferService:
    ...
    # --- 上傳邏輯 ---
    async def upload_files(self, parent_id: int, local_paths: List[str], concurrency_limit: int, progress_callback: Callable):
        client = await utils.ensure_client_connected(self.shared_state)
        if not client:
            logger.error("上傳無法啟動：連線無效。")
            for path in local_paths:
                progress_callback(f"ul_{uuid.uuid4()}", os.path.basename(path), 0, 0, 'failed', 0, message="連線失敗，無法啟動上傳。")
            return
        ...
        tasks_to_run = [
            self._upload_single_file(client, group_id, parent_id, file_path, semaphore, progress_callback)
            for file_path in local_paths
        ]
        await asyncio.gather(*tasks_to_run, return_exceptions=True)

    async def _upload_single_file(self, client, group_id, parent_id, file_path, semaphore, progress_callback):
        task_id = f"ul_{uuid.uuid4()}"
    ...
    async def download_items(self, items: List[Dict], destination_dir: str, concurrency_limit: int, progress_callback: Callable):
        ...
        tasks_to_run = []
        for item in items:
            # 在後端為每個頂層下載項生成一個唯一的 task_id
            main_task_id = f"dl_{uuid.uuid4()}"
            tasks_to_run.append(
                self._download_single_item(client, group_id, main_task_id, item, destination_dir, semaphore, progress_callback)
            )
        await asyncio.gather(*tasks_to_run, return_exceptions=True)
    ...
    async def _download_single_item(self, client, group_id, main_task_id: str, item: Dict, dest_path: str, semaphore, progress_callback):
        item_db_id, item_type, item_name = item['id'], item['type'], item['name']
        ...
        async with semaphore:
            ...
            try:
                if item_type == 'file':
                    ...
                    for f in folder_contents['items']:
                        if f['type'] == 'file':
                            files_in_folder.append({
                                ...
                                'task_id': f"dl_{uuid.uuid4()}" # 為每個子檔案生成 UUID task_id
                            })
                        elif f['type'] == 'folder':
                            # 為所有子資料夾建立本地目錄
                    ...
                    # 向前端發送 'starting_folder' 事件，包含所有子檔案的 task_id
                    child_info_for_frontend = [
                        {'id': f['task_id'], 'name': f['name'], 'size': f['size'], 'relative_path': f['relative_path']} 
                    ...
                    for f_details in files_in_folder:
                        ...
                        download_tasks.append(
                            self._download_file_from_details(client, group_id, f_details['task_id'], db.get_file_details(f_details['db_id']), file_dest_path, progress_callback, parent_task_id=main_task_id)
                        )
